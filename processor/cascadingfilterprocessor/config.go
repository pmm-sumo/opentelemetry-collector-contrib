...
const (
	...
	//add param with probabilistic sampling, specifying maximum number of spans to cover those
	//such rule must be applied always at the beginning of iteration and then calculations will be easy
	//the spans will be Sampled, obviously, regardless of other rule evaluations
	//
	//all spans that passed through the rule must have a correct attribute set
	//all spans that did not pass through that rule must have sampling.probability attribute removed
)
