...
func TestTraceIsDispatchedAfterDuration(t *testing.T) {
	...
	p, err := newGroupByAuthProcessor(logger, st, mockProcessor, config)
...
func TestProcessorCapabilities(t *testing.T) {
	...
	// test
	p, err := newGroupByAuthProcessor(logger, st, next, config)
...
func BenchmarkConsumeTracesCompleteOnFirstBatch(b *testing.B) {
	...
	p, err := newGroupByAuthProcessor(zap.NewNop(), st, next, config)
